...
export default function AlumniDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch("http://localhost:5000/alumni/users", {
          headers: {
            "auth-token": token,
          },
        });
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 p-6">
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-gray-300 pb-4 mb-6">
        ...
        <button onClick={handleLogout} className="bg-red-500 px-4 py-2 rounded-lg text-white hover:bg-red-600">
          Logout
        </button>
      </div>
